...
namespace SRTWrapper {
    ...
    bool SRTSocket::Create() {
        ...
        return SRT_C_IsValidSocket(m_socket);
    }
    
    bool SRTSocket::Close() {
        if (SRT_C_IsValidSocket(m_socket)) {
    ...
    bool SRTSocket::Bind(const char* ip, int port) {
        if (!SRT_C_IsValidSocket(m_socket)) return false;
    ...
    SRTSocket* SRTSocket::Accept() {
        if (!SRT_C_IsValidSocket(m_socket)) return nullptr;
        
        int client_sock = SRT_C_Accept(m_socket);
        if (!SRT_C_IsValidSocket(client_sock)) return nullptr;
    ...
    int SRTSocket::Send(const char* data, int len) {
        if (!SRT_C_IsValidSocket(m_socket)) return -1;
